...
function setFormData(){ 
    ...
    $query1 = $con->prepare("INSERT INTO `costumer`(gender,firstName,middleName,lastName,street,houseNumber,houseNumber_addon,zipCode,city,phone,password,newsletter_subscription,country,emailadress) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    
    
    $query1->bind_param('sssssisssssiss',$gender,$firstName,$middleName,$lastName,$street,$houseNumber,$houseNumber_addon,$zipcode,$city,$phone,$pwd_hased,$newsletter,$country,$email);
    ...
    if($query1->execute() === false) {
        ...
        $query1->execute();
        header('Location: ../product/product_overzicht.php');
        $query1->close();
    }
}
?>